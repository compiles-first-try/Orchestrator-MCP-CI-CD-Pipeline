...
5. **REST/OData over OAuth2 External App is the only Orchestrator transport for v1's data-plane operations.** Revised 2026-05-02 from the original "MCP-primary, REST-fallback" wording: research (`docs/research/uipath-mcp-orchestrator-surface-2026-05-02.md`) confirmed UiPath's MCP server exposes automations/agents/workflows, not Asset/Queue/Bucket/Credential CRUD. The audit-row `transport` enum is preserved for forward compat (v2's "Slack triggers automation" feature will reintroduce an MCP path for *invoking automations*, which is a separate concern from the reconciler's CRUD). v1 always logs `rest_fallback`. Tool discovery at boot is removed — there's nothing to discover for v1's scope.
