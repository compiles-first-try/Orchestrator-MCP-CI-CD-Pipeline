...
export class EncryptionError extends RpaPlatformError {
  constructor(reason: string, options: RpaPlatformErrorOptions = {}) {
    super("encryption.invalid", `Encryption error: ${reason}.`, options);
  }
}

export function parseEncryptionKey(base64Key: string): Buffer {
  if (typeof base64Key !== "string" || base64Key.length === 0) {
    throw new EncryptionError("encryption key is missing");
  }
  let buffer: Buffer;
  try {
...
export function encryptSecret(plaintext: string, key: Buffer): string {
  if (key.length !== KEY_BYTES) {
    throw new EncryptionError(`key length must be ${KEY_BYTES} bytes`);
  }
...
export function decryptSecret(envelope: string, key: Buffer): string {
  ...
  if (buffer.length < IV_BYTES + TAG_BYTES + 1) {
    throw new EncryptionError("ciphertext envelope is too short");
  }
  ...
  try {
    ...
    throw new EncryptionError("authentication tag verification failed", { cause });
  }
}
